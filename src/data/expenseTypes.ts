...
const initializeCategories = (): Record<string, ExpenseType> => {
  try {
    ...
    localStorage.setItem(CATEGORIES_STORAGE_KEY, JSON.stringify(DEFAULT_EXPENSE_TYPES));
...
export const EXPENSE_TYPES: Record<string, ExpenseType> = initializeCategories();

// Function to add a new category
export const addExpenseType = (newCategory: ExpenseType) => {
  EXPENSE_TYPES[newCategory.id] = newCategory;
  // Save to localStorage
  try {
    localStorage.setItem(CATEGORIES_STORAGE_KEY, JSON.stringify(EXPENSE_TYPES));
  } catch (error) {
    console.error('Error saving categories to localStorage:', error);
  }
};

// Function to generate a unique ID for new categories
export const generateCategoryId = (name: string): string => {
  const baseId = name.toLowerCase().replace(/\s+/g, '-');
  let id = baseId;
  let counter = 1;
  
  while (EXPENSE_TYPES[id]) {
    id = `${baseId}-${counter}`;
    counter++;
  }
...
export const getExpenseType = (typeId: string): ExpenseType => {
  return EXPENSE_TYPES[typeId] || EXPENSE_TYPES.groceries; // fallback to groceries
};

export const getExpenseTypeList = (): ExpenseType[] => {
  return Object.values(EXPENSE_TYPES);
};

// Function to refresh categories from localStorage (for when data changes)
export const refreshCategories = () => {
  const stored = localStorage.getItem(CATEGORIES_STORAGE_KEY);
  if (stored) {
    Object.assign(EXPENSE_TYPES, JSON.parse(stored));
  }
}; 